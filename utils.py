...
class Unet_network:
    def __init__(self, input_shape, out_ch, loss='dice_loss', metrics=['dice_coef', 'dis_dice_coef'], style='basic', ite=2, depth=4, dim=32, weights='', init='he_normal',acti='elu',lr=1e-4, dropout=0):
...
def make_callbacks(weight_name, history_name, monitor='val_loss', patience=100, mode='min', save_weights_only=True):
    ...
    reduce_lr=ReduceLROnPlateau(monitor='val_loss', factor=0.9, patience=5, verbose=0, min_lr=1e-8)
